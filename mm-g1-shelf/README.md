...
Walk with WASD, stand near the shelf, press **B**. The matcher cuts into the
pick clip at its best idle frame and the ride plays to the end. Because the
playback is relative (the clip is replayed from wherever the robot stands),
two things fix up the grab:
