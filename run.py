...
Drive the G1 with WASD, walk up to the shelf ahead, and press B: the matcher
transitions into the pick clip, the arm IK lands the palm on the vase, and
the vase sticks to the hand -- then walk away with it.
...
def main():
    ...
    ap.add_argument("--no-post-processing", action="store_true",
                    help="raw motion matching: skip the grab gate and the "
                         "reach-phase arm IK")
    args = ap.parse_args()
